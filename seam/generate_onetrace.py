...
def show():
    ...
    x = np.fromfile("/data/hany/0pinsan_dataset_test/x/00400.bin", dtype=np.float32)  # 1550x2001 double
    ...
    y = np.fromfile("/data/hany/0pinsan_dataset_test/y/00400.bin", dtype=np.float32)  # 1550x2001 double
...
def originalshow():
    ...
    x = np.fromfile("/data/hany/0pinsan_test_x/record_0.2ms_no_time_dispersion_shot102_2001x550.bin", dtype=np.float32).reshape(550,2001)[k,::]  # 1550x2001 double
    ax1 = plt.subplot(1, 1, 1)
    ...
    y = np.fromfile("/data/hany/0pinsan_test_y/record_1.2ms_shot102_2001x550.bin", dtype=np.float32).reshape(550, 2001)[k, ::]  # 1550x2001 double
...
if __name__ == '__main__':
    ...
    data_dir = "./data/2D/SEAM_time_train5"
    #data_dir_val = "./data/2D/SEAM_time_test7"

    #num_trainfile = glob.glob(r'./data/2D/SEAM_time_test7/*.bin')
    ...
    for i in range(0, len(list1)):
        for k in range(0, 325, 1):
            ...
            f1 = f1.reshape(325, 2001)
            data = f1[k, ::]
            #print("data image: ", data.shape)
            binPath = os.path.join('./data/1trace/train5shot/x', '%05d.bin' % (0+k+i*325))
            #binPath = os.path.join('./data/1trace/test7shot/x', '%05d.bin' % (0 + k + i * 325))
            data.tofile(binPath)
